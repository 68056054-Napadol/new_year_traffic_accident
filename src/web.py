...
st.title("🚨 ระบบพยากรณ์อุบัติเหตุบนท้องถนน")
...
def load_data():
    ...
    try:
        # โหลดข้อมูลพยากรณ์
        forecast_df = pd.read_csv(r'forecast_2025_2026.csv')
        forecast_df['adate'] = pd.to_datetime(forecast_df['adate'])
        
        # โหลดข้อมูลพิกัด
        coord_df = pd.read_csv(r'coordinate/tambon.csv')
...
if forecast_df is not None and amphoe_coord is not None:
    ...
    with tab2:
        ...
        with col4:
            st.metric("📈 ค่าเฉลี่ยต่อวัน", f"{filtered_df.groupby('adate')['predicted_cases'].sum().mean():.1f}")
        ...
        with col_graph1:
            ...
            daily_trend = filtered_df.groupby('adate')['predicted_cases'].sum().reset_index()
